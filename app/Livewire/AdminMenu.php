...
class AdminMenu extends Component
{
    ...
    public function addOptionItem($optionIndex)
    {
        $this->options[$optionIndex]["items"][] = [
            "name" => "",
            "additional_price" => 0,
        ];
    }
    public function removeOptionItem($optionIndex, $itemIndex)
    {
        unset($this->options[$optionIndex]["items"][$itemIndex]);
        $this->options[$optionIndex]["items"] = array_values(
            $this->options[$optionIndex]["items"],
        );
    }
    ...
    public function saveMenu()
    {
        ...
        try {
            ...
            session()->flash(
                "success",
                $this->menu_id ? "Menu diperbarui!" : "Menu ditambahkan!",
            );
            $this->closeModal();
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function render()
    {
        ...
        if ($this->activeTab == "menu") {
            ...
            $menus = null;
            $categoriesList = null;
            $paginatedCategories = Category::where(
                "name",
                "like",
                "%" . $this->search . "%",
            )
        ...
        return view(
            "components.admin.admin-menu",
            compact("menus", "categoriesList", "paginatedCategories"),
