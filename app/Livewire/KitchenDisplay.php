...
class KitchenDisplay extends Component
{
    ...
    public function updateStatus($orderId, $status)
    {
        ...
        if ($order) {
            ...
            session()->flash('success', "Pesanan Meja {$order->table->table_number} {$pesan}!");
        }
    }
    ...
    public function render()
    {
        ...
        $orders = Order::with(['table', 'items' => function($q) {
                $q->whereIn('item_status', ['pending', 'cooking'])
                  ->with('menu', 'selectedOptions');
            }])
        ...
        return view('components.kitchen.kitchen-display', compact('orders'))
